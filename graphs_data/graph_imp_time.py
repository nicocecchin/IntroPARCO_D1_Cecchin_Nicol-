...
def create_plots_from_csv(file_path):
    ...
    plt.title('Average Time for Checksym vs Matrix Size (Implicit parallelization)')
    plt.grid(True)
    plt.legend()
    plt.xscale('log')
    plt.yscale('log')
    ...
    plt.title('Average Time for Transposition vs Matrix Size (Implicit parallelization)')
    plt.grid(True)
    plt.legend()
    plt.xscale('log')  
    plt.yscale('log')
    plt.tight_layout()
    plt.savefig("transpose_imp.png")
    plt.show()

file_path = 'implicit_results_time.csv'
